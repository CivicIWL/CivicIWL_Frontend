...
export function useIncidents() {
  ...
  const createIncident = useCallback(
    async (
      incident: Omit<
        Incident,
        "id" | "userId" | "submittedOn" | "lastUpdated" | "status"
      >,
    ) => {
      setLoading(true);
      setError(null);
  ...
  const getIncidents = useCallback(
    async (params?: {
      ...
      try {
        ...
        const errorMessage =
          err.response?.data?.error || "Failed to fetch incidents";
        setError(errorMessage);
        throw new Error(errorMessage);
      } finally {
        setLoading(false);
  ...
  const updateIncidentStatus = useCallback(
    async (id: string, status: string, notes?: string, assignedTo?: string) => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const errorMessage =
          err.response?.data?.error || "Failed to update incident";
        setError(errorMessage);
        throw new Error(errorMessage);
      } finally {
        setLoading(false);
