...
export function StaffDashboard({ user }: StaffDashboardProps) {
  ...
  const fetchDashboardData = async (isRefresh = false) => {
    try {
      ...
      const metricsData =
        incidentMetrics.status === "fulfilled" ? incidentMetrics.value : null;
      ...
      if (
        recentIncidents.status === "fulfilled" &&
      ...
      if (activities.length === 0) {
        ...
        if (hasSuccessfulApiCalls) {
          ...
          activities.push({
            ...
            action: "System ready",
            details:
              "Your city management system is active and ready to receive reports",
            time: "Now",
            type: "knowledge",
          });
        } else {
      ...
      if (
        activities.length > 0 &&
        activities[0].id !== "system-ready" &&
        activities[0].id !== "system-loading"
      ) {
        activities.sort((a, b) => {
          const timeA =
            a.time === "Now" ? Date.now() : new Date(a.time).getTime();
          const timeB =
            b.time === "Now" ? Date.now() : new Date(b.time).getTime();
          return timeB - timeA;
        });
      }
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-600">
              <span
                className={
                  stats.newReportsChange >= 0
                    ? "text-orange-600"
                    : "text-green-600"
                }
              >
                {stats.newReportsChange >= 0 ? "+" : ""}
                {stats.newReportsChange}
              </span>{" "}
              since yesterday
            </p>
          </CardContent>
            ...
            <div className="space-y-4">
              {recentActivities.length > 0 ? (
                recentActivities.map((activity) => (
                  <div
                    key={activity.id}
                    className="flex items-start space-x-3 p-3 rounded-[3rem] bg-slate-50 hover:bg-slate-100 transition-colors"
                  >
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm mt-1">
                        When incidents are reported or users register, they'll
                        appear here
                      </p>
                    </div>
